...
public class ScrollAndZoom : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (Input.touchCount >= 2)
        {
            ...
            if ( Camera.transform.position.y > (cameraStartPosition.y + CameraUpperHeightBound) || Camera.transform.position.y > 400)
            {
                Camera.transform.position = camPositionBeforeAdjustment;
            }
            //Lower (Zoom in)
            if (Camera.transform.position.y < 400 && Camera.transform.position.y < (cameraStartPosition.y - CameraLowerHeightBound) || Camera.transform.position.y <= 1)
            {
                Camera.transform.position = camPositionBeforeAdjustment;
            }
